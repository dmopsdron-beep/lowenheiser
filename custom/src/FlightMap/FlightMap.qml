...
Map {
    ...
    // CUSTOM Löweheiser: el marcador GCS solo se muestra cuando hay rumbo conocido
    // (flecha pequeña). Sin rumbo (caso laptop) ocultamos el marcador para no pintar
    // el logo grande en el centro del mapa.
    MapQuickItem {
        anchorPoint.x:  sourceItem.width / 2
        anchorPoint.y:  sourceItem.height / 2
        visible:        gcsPosition.isValid && !planView && !isNaN(gcsHeading)
